...
import { DropdownButton } from "~/app/components/clickable";
...
export default function CodeGen({
  ...
  const { manageNewComponentFocus, addNewComponents } =
  ...
  const updateComponent = api.component.update.useMutation({
    onMutate: async ({ content, id }) => {
      ...
      const previousFile = utils.file.getFile.getData({ id: file.id });

      utils.file.getFile.setData({ id: file.id }, (oldQueryData) => {
        if (oldQueryData && content) {
          ...
          addNewComponents(updatedComponents);
  ...
  const saveResponseMenuRef = useRef<HTMLButtonElement>(null);

  useEffect(() => {
    if (isRefetching || !!stream) {
                  ...
                  <>
                    ...
                    <form
                      ...
                      onSubmit={async (e) => {
                        ...
                        setCalled(false);
                        setInput("");
                        saveResponseMenuRef.current?.click();
                      }}
                    >
                      <Textarea
                        autoFocus
